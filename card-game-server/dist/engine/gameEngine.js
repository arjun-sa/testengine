...
export function createGame(playerCount, totalRounds = 8) {
...
export function selectPair(state, playerId, cards) {
    ...
    for (const card of cards) {
        if (!player.hand.includes(card))
            return state;
        if (player.benchedCards.includes(card))
            return state;
    }
    if (cards[0] === cards[1])
        return state;
    const newPlayers = state.players.map((p) => p.id === playerId ? { ...p, selectedPair: cards } : p);
...
export function startChoose(state) {
    if (state.phase !== 'reveal')
        return state;
    return {
        ...state,
...
export function chooseCard(state, playerId, card) {
    ...
    if (!player.selectedPair.includes(card))
        return state;
