...
export class LobbyManager {
    ...
    maxRooms;
    rooms = new Map();
    constructor(connectionManager, maxRooms = 100) {
        this.connectionManager = connectionManager;
        this.maxRooms = maxRooms;
    }
    createRoom(connection, playerName) {
    ...
    joinRoom(connection, roomCode, playerName) {
        if (connection.roomCode) {
            this.sendError(connection, 'INVALID_ACTION', 'Already in a room');
            return;
        }
    ...
    handleGameAction(connection, message) {
        const room = this.getConnectionRoom(connection);
        if (!room)
            return;
    ...
    handleDisconnect(connection) {
        ...
        logger.info({ roomCode: connection.roomCode, playerId: connection.playerId }, 'Player disconnected');
    }
    handleReconnect(connection) {
        ...
        logger.info({ roomCode: connection.roomCode, playerId: connection.playerId }, 'Player reconnected');
    }
    getRoom(code) {
